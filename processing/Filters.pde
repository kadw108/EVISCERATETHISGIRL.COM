// From https://github.com/anartisticengineer/Glitch-FX-Processing-Version/blob/master/Glitch_FX/Filters.pde
...
class Filters {
  ...
  PImage scanlines(PImage src_) {
    ...
    for (int x = 0; x < width; x+=(dir)?1:2) {
      for (int y = 0; y < height; y+=(dir)?2:1) {
        if (alpha(src_.pixels[x + y* width]) != 0) {
  ...
  PImage jpgDegrade(PImage src_) {
    PImage s_copy = src_.copy();
    PImage dest = s_copy;
    int scaleBy = int(pow(2, round(random(1, 2)))); //2,4,8,16
